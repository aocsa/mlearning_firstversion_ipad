...
namespace MLearning.Touch.CustomComponents
{
	...
	public class MLOItemView : UIView
	{
		...
		public MLOItemView (int i, float posX, string title, string name, string lastname, bool lMe) : base ()
		{
			...
			Frame = new RectangleF (posX, 0, 122, 142);
			Layer.CornerRadius = 2;

			MloContainer = new UIView (new RectangleF(3, 3, 116, 136));
			MloContainer.Layer.CornerRadius = Constants.MloCornerRadius;

			MloImage = new UIImageView (new RectangleF (0, 0, 116, 136));
			MloImage.Layer.MasksToBounds = true;
			MloImage.Layer.CornerRadius = Constants.MloCornerRadius;
			MloContainer.Add (MloImage);

			DescriptionBackground = new UIView (new RectangleF(0, 78, 116, 58));
			...
			DescriptionLabel = Constants.makeLabel (new RectangleF (5, 5, 116, 33), UIColor.White, UITextAlignment.Left, Font.Bold, 12);
			...
			LikesLabel = Constants.makeLabel (new RectangleF (56, 38, 52, 16), UIColor.White, UITextAlignment.Center, Font.Regular, 9);
